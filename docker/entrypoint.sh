...
while [ "$COMPTEUR" -lt "$LIMITE" ]; do
    if php /usr/local/bin/attend_db.php "$DB_HOST" "$DB_PORT" "$DB_USERNAME" "$DB_PASSWORD" >/dev/null 2>&1; then
        break
    fi
    COMPTEUR=$((COMPTEUR + 1))
    sleep 2
...
if [ "$COMPTEUR" -ge "$LIMITE" ]; then
    echo "ERREUR : base injoignable après 60 s. Vérifiez les variables DB_HOST, DB_PORT, DB_DATABASE, DB_USERNAME, DB_PASSWORD." >&2
    exit 1
fi
